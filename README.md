...
Download the pre-trained network models and move to `tipdircnn/weights/`.

## 3. Train network
After preparing the dataset, train tipdircnn with the following command:
```bash
# For Example
...
# Using dircen, which predicts quality with variance of Cos/Sin.
...
In order to easily integrate our system with other programming languages, we set up a socket-based server for the inference-optimization process in `optimization/grasp_generation_server.py`, which can receive RGBD/Depth images via IP/Port settings, and return the estimated grasp pose for grippers.

To use the inference-optimization hybrid nodes, first start the server, which listens to the desired IP:Port indicated in `optimization/modules/param_set.py` (this script stores all settings of the grasp generation, check before use). Then, another client node (`optimization/testset_eval_grippertype_client.py` for ten grippers) can connect to the server and send images to the server node. After inferring and generating the grasp pose, the server node returns the grasp pose (position, orientation, and other properties) to the client node. Following are the detailed steps:

- Step 1: Check the grasp generation settings in `optimization/modules/param_set.py`, including the *IP:Port*, *grippertype*, *network weights*, *TTA Size* and *Input type*, etc.
...
- Step 3: Start the image client node, which retrieves the Cornell dataset and publishes the image data via socket to the server node:
```bash
# Start image client
# Conda env: conda activate hybridgenEnv
python optimization/testset_eval_grippertype_client.py
```
- Step 4: Server node will conduct inference and optimization (within one minute), and return the optimization results to the image client. The results are stored at the path stated by`result_dir` at `param_set.py`, initially set as `tipdircnn/output/`. 
