...
def TTA_modeling_visual(tta_samples, label_map):
    regulated_sample_data = {kn : [] for kn in ['Depth', 'RGB', 'Probability', 'Cos', 'Sin', 'Probability_T', 'Cos_T', 'Sin_T', 'Cos_TG', 'Sin_TG']} # 'Angle_Ori', 
    ...
    for row_i in range(len(tta_samples)):
        ...
        dep = Image(tta_samples[row_i]['img'][0])
        # rgb = Image(np.mean(tta_samples[row_i]['img'][1:], 0).squeeze())
        rgb_ori = tta_samples[row_i]['img'][1:]
        rgb = Image(np.transpose(rgb_ori - rgb_ori.min(), (1, 2, 0)))
        
        # Visually rotation
        cos_img_np_ori = np.array(Image(tta_samples[row_i]['pred']['cos']))
        sin_img_np_ori = np.array(Image(tta_samples[row_i]['pred']['sin']))
        ...
        angle_img_np = np.array(Image(tta_samples[row_i]['pred']['ang']))
        ...
        regulated_sample_data['Depth'].append(dep.copy())
        regulated_sample_data['RGB'].append(rgb.copy())
        regulated_sample_data['Cos'].append(cos_img_np_ori.copy())
        regulated_sample_data['Sin'].append(sin_img_np_ori.copy())
        regulated_sample_data['Cos_T'].append(cos_img_np.copy())
        regulated_sample_data['Sin_T'].append(sin_img_np.copy())
        regulated_sample_data['Cos_TG'].append(cos_img_triangle.copy())
        regulated_sample_data['Sin_TG'].append(sin_img_triangle.copy())
        # regulated_sample_data['Angle_Ori'].append(angle_img_np.copy())

        if label_map['Qua'] != 'Non':
            quality_img_np_ori = np.array(Image(tta_samples[row_i]['pred']['qua']))
            ...
            quality_img_np_ori = quality_img_np
            
        regulated_sample_data['Probability'].append(quality_img_np_ori.copy())
        regulated_sample_data['Probability_T'].append(quality_img_np.copy())
    ...
    for (key, value) in regulated_sample_data.items():
        ...
        if key == 'Probability_T': key = 'qua'
        if key == 'Cos_TG': key = 'cT'
        if key == 'Sin_TG': key = 'sT'
    ...
    ### Temp solution
    tta_estimation['qua_ori'] = regulated_sample_data['Probability'][0]
    tta_estimation['ang_ori'] = np.arctan2(regulated_sample_data['Sin'][0], regulated_sample_data['Cos'][0])\
         / (2.0 if (label_map['Cos'] == 'Pos') else 1.0)
    
    tta_estimation['wid' + '_mean'] = np.ones_like(cos_img_np) * label_map['detect']['wid']
    tta_estimation['wid' + '_amax'] = np.ones_like(cos_img_np) * label_map['detect']['wid']
