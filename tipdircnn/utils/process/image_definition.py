...
class Image:
    ...
    def __getattr__(self, attr):
        return getattr(self.img, attr) # Get the attribute of class attribute
    ...
    def copy(self):
        ...
        :return: Copy of self.
        """
        return self.__class__(self.img.copy()) # Return a self class object

    def crop(self, top_left, botton_right, resize=None):
        """
    ...
    def rotate(self, angle, center=None):
        ...
        self.img = rotate(self.img, angle/np.pi*180.0, center=center, mode='symmetric', 
    ...
    def zoom(self, factor):
        ...
        """
        sr = int(self.img.shape[0] * (1 - factor)) // 2 # left
        sc = int(self.img.shape[1] * (1 - factor)) // 2 # up
        orig_shape = self.img.shape
        self.img = self.img[sr:self.img.shape[0] - sr, sc: self.img.shape[1] - sc].copy() # copy out
        self.img = resize(self.img, orig_shape, mode='symmetric', 
            preserve_range=True).astype(self.img.dtype) # 放大到原来的尺寸

    def zoomed(self, *args, **kwargs): # 同 cropped
        """
...
class DepthImage(Image):
    ...
    @classmethod
    def from_pcd(cls, pcd_filename, shape, default_filter=0, index=None):
        """
        ...
        with open(pcd_filename) as f:
            for l in f.readlines():
                ...
                else: # index means there is a depth data directly
                    img[r, c] = float(ls[index])
        return cls(img/1000.0) # metric m
    ...
    def inpaint(self, missing_value=0):
        ...
        self.img = self.img.astype(np.float32) / scale  # Has be float32, 64 not supported. get -1:1
        self.img = cv2.inpaint(self.img, mask, 1, cv2.INPAINT_NS) # repair with fluid alg. radius 1

        # Back to original size and value range.
        self.img = self.img[1:-1, 1:-1] # cut the 1 pixel boarder
    ...
    def gradients(self):
        ...
        """
        grad_x = cv2.Sobel(self.img, cv2.CV_64F, 1, 0, borderType=cv2.BORDER_DEFAULT) #1,0 si dx, dy
        grad_y = cv2.Sobel(self.img, cv2.CV_64F, 0, 1, borderType=cv2.BORDER_DEFAULT)
        grad = np.sqrt(grad_x ** 2 + grad_y ** 2) # combine gradient in x and y axises

        return DepthImage(grad_x), DepthImage(grad_y), DepthImage(grad) # return self.object!
...
class WidthImage(Image):
    ...
    def zoom(self, factor):
        ...
        :param factor: Factor to zoom by. e.g. 0.5 will keep the center 50% of the image.
        """
        super().zoom(factor) # super() is the base class 
        self.img = self.img/factor

    def normalise(self, width_scale=150.0):
        """
        Normalise by mapping [0, 150] -> [0, 1]
        """
        self.img = np.clip(self.img, 0, width_scale)/width_scale
